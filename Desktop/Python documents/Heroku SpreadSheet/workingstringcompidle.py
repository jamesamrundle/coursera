...
def inventoryproducts():#generates list of items contained in inventory sheet and associated count(at same index value)
    ...
    for i in range(6,123):
        ...
        invcount.append(str(ws['f'+str(i)].value)+' '+str(ws['d'+str(i)].value) )
...
goaldict = {}## dict containing product: par:val and inv:val

def printdata():
    
    for i in par4onhand:
        #par = str(par4onhand[i])
        #Onhand =str(actualonhand[i])
        print(i+ '--Par is: ' + str(par4onhand[i]) +' With :'+ str(actualonhand[i])+ ' --In current inventory')
        goaldict[i] = {'Par': par4onhand[i] , 'Onhand': actualonhand[i] }
        #goaldict[i] = {'par': '', 'onhand': ''}
printdata()        
